  ...
  try {
    ...
    for (var i = 0; i < testname.length; i++) {
      ...
      let patientData = {
        ...
        HMOId: patientObj?.HMOId,
        HMOName: patientObj?.HMOName
      };

      if (foundPatient?.isHMOCover == configuration.ishmo[0] || (appointment.patient).isHMOCover == configuration.ishmo[0]) {

        testrecord = await createlab({ testname: testname[i], patient: appointment.patient, appointment: appointment._id, appointmentid: appointment.appointmentid, testid, department, amount: Number(testPrice.amount), ...patientData });
      }
      else {

        testrecord = await createlab({ testname: testname[i], patient: appointment.patient, appointment: appointment._id, appointmentid: appointment.appointmentid, testid, department, amount: ((100 - Number(testPrice.percentageofhmocover)) / 100) * Number(testPrice.amount), ...patientData });
      }
